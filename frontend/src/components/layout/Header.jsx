...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-zinc-100/90 border-b border-zinc-200/80 backdrop-blur-md">
      <Container>
    <div className="relative"> {/* Added wrapper to help position the drop bar */}
      <nav className="flex h-22 items-center justify-between px-2">
        ...
        {/* 📱 Mobile Trigger Menu Controls */}
        <div className="flex md:hidden items-center gap-2">
          {/* 🎯 Toggle state for mobile view specifically */}
          <button 
            onClick={() => setIsSearchOpen(!isSearchOpen)} 
            className="p-2 text-zinc-700 hover:text-mauve-500 cursor-pointer"
          >
            <Search size={22} />
          </button>
